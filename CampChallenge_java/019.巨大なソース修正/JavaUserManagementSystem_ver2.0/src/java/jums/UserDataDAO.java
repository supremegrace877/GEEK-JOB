...
//  更新処理  
    ...
    public void update(UserDataDTO ud,HttpServletRequest request) throws SQLException{
        Connection con = null;
        PreparedStatement st = null;
        ...
        try{
            ...
            int type = ud.getType();
            String tell = ud.getTell();
            String comment = ud.getComment();
            int userID = (Integer)(session.getAttribute("userID"));
            
            st.setString(1,name);
            java.sql.Date sqlbirthday = new java.sql.Date(birthday.getTime());
            st.setDate(2, sqlbirthday);
            st.setInt(3,type);
            st.setString(4,tell);
            st.setString(5,comment);
            st.setInt(6,userID);
                
            int update_rows = st.executeUpdate();
            st.executeUpdate();
    ...
    //削除処理
    public void delete(UserDataDTO ud,HttpServletRequest request) throws SQLException{
        Connection con = null;
        PreparedStatement st = null;
        HttpSession session = request.getSession();
        try{
            ...
            int userID = (Integer)(session.getAttribute("userID"));
            st.setInt(1,userID);
            int delete_rows = st.executeUpdate();
            st.executeUpdate();
    ...
    public UserDataDTO search(UserDataDTO ud) throws SQLException{
        ...
        try{
            ...
            //結果がひとつも返ってこなければ、そのままresultsetを返す
